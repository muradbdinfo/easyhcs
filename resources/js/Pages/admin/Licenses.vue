...
const licenses = ref({ data: [], links: [], meta: {} })
const modal    = ref(false)
const loading  = ref(false)
const form     = ref({ tenant_id: '', expires_at: '', max_users: '' })

const statusColor = (s) => ({ active:'green', revoked:'red', expired:'gray' }[s] ?? 'gray')

async function fetch() {
  const { data } = await axios.get('/api/admin/licenses')
  licenses.value = data
}
fetch()
...
async function submit() {
  loading.value = true
  try {
    await axios.post('/api/admin/licenses', form.value)
    modal.value = false
...
async function revoke(lic) {
  if (!confirm(`Revoke ${lic.key}?`)) return
  await axios.post(`/api/admin/licenses/${lic.id}/revoke`)
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-if="!licenses.data.length"><td colspan="6" class="px-5 py-10 text-center text-gray-400">No licenses yet.</td></tr>
          <tr v-for="lic in licenses.data" :key="lic.id" class="hover:bg-gray-50">
            ...
            <td class="px-5 py-3"><AppBadge :color="statusColor(lic.status)">{{ lic.status }}</AppBadge></td>
            <td class="px-5 py-3 text-gray-400">{{ lic.expires_at ? new Date(lic.expires_at).toLocaleDateString() : 'Never' }}</td>
            <td class="px-5 py-3 text-gray-500">{{ lic.max_users ?? '∞' }}</td>
            <td class="px-5 py-3 text-right">
              <button v-if="lic.status === 'active'" @click="revoke(lic)" class="p-1.5 rounded hover:bg-red-50 text-red-500">
                <XCircle :size="15" />
              </button>
            </td>
          </tr>
      ...
      <div class="px-5 py-3 border-t border-gray-100"><AppPagination :links="licenses.links" :meta="licenses.meta" /></div>
    </div>
    ...
    <div v-if="modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div class="bg-white rounded-2xl w-full max-w-sm mx-4 shadow-2xl">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          ...
          <button @click="modal = false" class="p-1.5 rounded hover:bg-gray-100 text-gray-400"><X :size="18" /></button>
        </div>
        <form @submit.prevent="submit" class="p-6 space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Tenant ID</label>
            <input v-model="form.tenant_id" required class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Expires At (optional)</label>
            <input v-model="form.expires_at" type="date" class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Max Users (optional)</label>
            <input v-model="form.max_users" type="number" min="1" class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" />
          </div>
