...
const openCreate = () => { Object.assign(form, blankForm()); modal.value = 'create' }
const openEdit = (p) => { 
  Object.assign(form, { ...p, modules: p.modules_enabled ?? [] })
  modal.value = p 
}
...
async function save() {
  ...
  try {
    ...
    toast.success(modal.value === 'create' ? 'Plan created.' : 'Plan updated.')
    modal.value = null
    fetchPlans()
  } catch (err) {
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
      <div v-for="plan in plans" :key="plan.id" class="bg-white rounded-xl border border-gray-200 p-5 flex flex-col">
        ...
        <div class="flex flex-wrap gap-1.5 mb-3">
          <span v-for="mod in plan.modules_enabled" :key="mod" class="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full capitalize">{{ mod }}</span>
        </div>
        <p class="text-xs text-gray-400 mt-auto mb-3">{{ plan.subscriptions_count ?? 0 }} subscribers</p>
        <div class="flex gap-2 pt-3 border-t border-gray-100">
          <button @click="openEdit(plan)" class="flex-1 flex items-center justify-center gap-1.5 text-xs py-1.5 rounded-lg border border-gray-300 hover:bg-gray-50 transition text-gray-600">
            <Edit2 :size="13" /> Edit
          </button>
          <button @click="confirmDelete = plan" class="flex items-center justify-center px-3 text-xs py-1.5 rounded-lg border border-red-200 hover:bg-red-50 transition text-red-500">
            <Trash2 :size="13" />
          </button>
        </div>
      </div>
    ...
    <div v-if="modal !== null" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div class="bg-white rounded-2xl w-full max-w-lg mx-4 shadow-2xl">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          ...
          <button @click="modal = null" class="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400"><X :size="18" /></button>
        </div>
        <form @submit.prevent="save" class="p-6 space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Name</label>
            <input v-model="form.name" type="text" required class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
            <input v-model="form.description" type="text" class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" />
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Monthly Price ($)</label>
              <input v-model="form.price_monthly" type="number" step="0.01" min="0" required class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Yearly Price ($)</label>
              <input v-model="form.price_yearly" type="number" step="0.01" min="0" required class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Modules</label>
            <div class="flex flex-wrap gap-2">
              <label v-for="mod in allModules" :key="mod" class="flex items-center gap-2 cursor-pointer">
          ...
          <div class="flex items-center gap-2">
            ...
            <label for="is_active" class="text-sm text-gray-700">Active (visible to tenants)</label>
          </div>
          <div class="flex gap-3 pt-2">
            <button type="submit" :disabled="loading" class="flex-1 bg-blue-600 text-white py-2 rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50 transition">
              {{ loading ? 'Saving…' : 'Save Plan' }}
            </button>
            <button type="button" @click="modal = null" class="flex-1 border border-gray-300 py-2 rounded-lg text-sm font-medium hover:bg-gray-50 transition">Cancel</button>
          </div>
        </form>
      </div>
    </div>
